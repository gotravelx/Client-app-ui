// API utility functions for flight data
...
export async function fetchHistoricalFlightData(
  ...
  try {
    const response = await fetch(
      `${getBaseUrl()}/flights/fetch-historical/${flightNumber}/date-range?fromDate=${fromDate}&toDate=${toDate}&carrierCode=${carrierCode}`,
      {
        method: "GET",
        headers: {
...
export async function decryptFlightData(encryptedData: string[]) {
  try {
    const response = await fetch(`${getBaseUrl()}/flights/decrypt-flight-data`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
